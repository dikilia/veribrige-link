...
<body><div class="login-box"><h1>🔐 Admin Login</h1><input type="password" id="password" placeholder="Enter password"><button onclick="login()">Login</button><div id="error" class="error"></div></div>
<script>async function login(){const pwd=document.getElementById('password').value;const res=await fetch('/admin/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:pwd})});const data=await res.json();if(data.success){window.location.href='/admin/dashboard'}else{document.getElementById('error').innerText='Invalid password'}}</script>
</body></html>
    `);
});
...
<script>
let currentLinks=[],currentEditCode=null;
async function loadLinks(){const search=document.getElementById('search').value;const url=search?'/admin/api/links?search='+encodeURIComponent(search):'/admin/api/links';const res=await fetch(url);if(res.status===401){window.location.href='/admin';return}const data=await res.json();if(data.success){currentLinks=data.links;document.getElementById('totalLinks').innerText=currentLinks.length;document.getElementById('totalClicks').innerText=currentLinks.reduce((s,l)=>s+(l.clicks||0),0);renderTable()}}
function renderTable(){const tbody=document.getElementById('tableBody');if(currentLinks.length===0){tbody.innerHTML='<tr><td colspan="5" style="text-align:center">No links found</td></tr>';return}tbody.innerHTML=currentLinks.map(link=>'<tr><td><code>'+link.code+'</code></td><td>'+link.targetUrl+'</td><td>'+(link.clicks||0)+'</td><td>'+new Date(link.createdAt).toLocaleDateString()+'</td><td><button class="edit-btn" onclick="openEdit(\\''+link.code+'\\')">Edit</button> <button class="delete-btn" onclick="deleteLink(\\''+link.code+'\\')">Delete</button></td></tr>').join('')}
function openEdit(code){const link=currentLinks.find(l=>l.code===code);if(link){currentEditCode=code;document.getElementById('editCode').value=code;document.getElementById('editUrl').value=link.targetUrl;document.getElementById('editModal').style.display='flex'}}
...
document.getElementById('search').addEventListener('input',()=>{clearTimeout(window.searchTimeout);window.searchTimeout=setTimeout(loadLinks,500)});loadLinks();setInterval(loadLinks,30000);
</script></body></html>
    `);
});
...
<body class="bg-black text-white">
        ...
        <div class="w-full max-w-2xl text-center">
            <div class="bg-black/40 backdrop-blur-xl border border-cyan-500/30 rounded-2xl p-8">
                ...
                <div class="space-y-4">
                    ...
                    <div class="text-xs text-white/30 pt-4 border-t border-white/10"><p>✅ <span class="text-green-400">VALIDATED URLs!</span> Only allowed Roblox domains work</p><p>🔗 Links never expire - stored securely</p></div>
                </div>
            </div>
        </div>
    </div>
    <script>
        ...
        async function loadAllowedDomains(){try{const res=await fetch('/api/allowed-domains'),data=await res.json();if(data.success)domainsList.innerHTML=data.domains.map(d=>'<span>'+d+'</span>').join('')}catch(err){domainsList.innerHTML='<span>roblox.com</span><span>www.roblox.com</span>'}}
        loadAllowedDomains();
        async function generateLink(){let url=targetUrl.value.trim();if(!url){errorMsg.innerText='❌ Please enter a URL';return}errorMsg.innerText='';generateBtn.disabled=true;generateBtn.innerHTML='<div class="loading-spinner"></div> Generating...';try{const response=await fetch('/api/generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:url})}),data=await response.json();if(data.success){resultUrl.innerText=data.shareableLink;resultSection.classList.remove('hidden')}else{errorMsg.innerText='❌ '+data.error;resultSection.classList.add('hidden')}}catch(err){errorMsg.innerText='❌ Server error';resultSection.classList.add('hidden')}finally{generateBtn.disabled=false;generateBtn.innerHTML='Generate Shareable Link'}}
        generateBtn.addEventListener('click',generateLink);copyBtn.addEventListener('click',()=>{navigator.clipboard.writeText(resultUrl.innerText)});testBtn.addEventListener('click',()=>{window.open(resultUrl.innerText,'_blank')});targetUrl.addEventListener('keypress',e=>{if(e.key==='Enter')generateLink()});
    </script>
</body>
</html>
    `);
...
app.get('/verify.html', (req, res) => {
    ...
    // Your beautiful verify.html - with FIXED async issue
...
<script>
// ==================== FIXED: TARGET_URL now loads correctly ====================
...
function showRobloxFlow(){
    // FIXED: Uses the loaded TARGET_URL (not the default)
    const urlToLoad = TARGET_URL;
    console.log('[VeriBridge] Loading URL in iframe:', urlToLoad);
    robloxIframe.src = urlToLoad;
...
// Dashboard logic (keep your existing dashboard code)
